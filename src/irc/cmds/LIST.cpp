...
void    IRC::LIST( const Cmd & cmd, vector<t_ClientMsg> & res )
{
    if (cmd._params.empty())
    {
        ...
        PushToRes(cmd._user->getFd(), getServReply(cmd._user,  RPL_LISTSTART, NULL), res);
        for (map<string, Channel *>::iterator it = _chanList.begin() ;
            it != _chanList.end() ; ++it )
        {
            Channel *chan = it->second;
            PushToRes(cmd._user->getFd(), getServReply(cmd._user,  RPL_LIST, (string[]){chan->getName(),
    ...
    PushToRes(cmd._user->getFd(), getServReply(cmd._user, RPL_LISTEND, NULL), res);
}