...
void    IRC::INVITE( const Cmd & cmd, vector<t_ClientMsg> & res )
{
    ...
    else
    {
        ...
        if (chan == NULL)
            PushToRes(_user->_fd, getServReply(_user,  ERR_NOSUCHCHANNEL, (string[]){ cmd._params[1] }), res);
        else
        {
            ...
            else
            {
                ...
                if (invited->_joined.find(chan) != _user->_joined.end())
                    PushToRes(_user->_fd, getServReply(_user, ERR_USERONCHANNEL, (string[]){cmd._params[0], _params[1]}), res);
                //  ERR if the requesting user is not a member of the channer
                else if (_user->_joined.find(chan) == _user->_joined.end())
                    PushToRes(_user->_fd, getServReply(_user, ERR_NOTONCHANNEL, (string[]){cmd._params[1] }), res);
                //  ERR if the channel has invite-only mode set, and the user is not a channel operator.
                    ...
                    PushToRes(_user->_fd, getServReply(_user, ERR_CHANOPRIVSNEEDED, (string[]){cmd._params[1]}), res);
                else
                {
                    ...
                    PushToRes(_user->_fd, getServReply(_user, RPL_INVITING, (string[]){cmd._params[0],cmd._params[1]}), res);
                    //  Msg to the invited user
                    PushToRes(invited->_fd, getServReply(_user, RPL_INVITING, (string[]){cmd._params[0],cmd._params[1], _user->getNick()}), res);
                }
            }
        }
    }
