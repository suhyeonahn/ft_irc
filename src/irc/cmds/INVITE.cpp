...
void    IRC::INVITE( const Cmd & cmd, vector<t_ClientMsg> & res )
{
    ...
    else
    {
        ...
        if (chan == NULL)
            PushToRes(cmd._user->_fd, getServReply(cmd._user,  ERR_NOSUCHCHANNEL, (string[]){ cmd._params[1] }), res);
        else
        {
            ...
            //  Not defined by MAN..
            if (invited == NULL)
                PushToRes(cmd._user->_fd, getServReply(cmd._user, ERR_NOSUCHNICK, (string[]){cmd._params[0]}), res);
            else
            {
                //  ERR if the invited user is on the channel already
                if (invited->_joined.find(chan) != cmd._user->_joined.end())
                    PushToRes(cmd._user->_fd, getServReply(cmd._user, ERR_USERONCHANNEL, (string[]){cmd._params[0],cmd._params[1]}), res);
                //  ERR if the requesting user is not a member of the channer
                else if (cmd._user->_joined.find(chan) == cmd._user->_joined.end())
                    PushToRes(cmd._user->_fd, getServReply(cmd._user, ERR_NOTONCHANNEL, (string[]){cmd._params[1] }), res);
                //  ERR if the channel has invite-only mode set, and the user is not a channel operator.
                    ...
                    PushToRes(cmd._user->_fd, getServReply(cmd._user, ERR_CHANOPRIVSNEEDED, (string[]){cmd._params[1]}), res);
                else
                {
                    ...
                    PushToRes(cmd._user->_fd, getServReply(cmd._user, RPL_INVITING, (string[]){cmd._params[0],cmd._params[1]}), res);
                    //  Msg to the invited user
                    PushToRes(invited->_fd, getServReply(cmd._user, RPL_INVITING, (string[]){cmd._params[0],cmd._params[1], cmd._user->getNick()}), res);
                }
            }
        }
    }
