            ...
            if (targets[i].find(CHAN_PREFIX) != string::npos)
            {
                ...
                else
                {
                    ...
                    if (cmd._params[0][0] == OPER_PREFIX)
                        Emit2(chan->_operList, getServMsg(cmd._user, MSG_PRIVMSG, (string[]){chan->getName(),cmd._params[1]}), res);
                    else
                        Emit2(chan->_userList, getServMsg(cmd._user, MSG_PRIVMSG, (string[]){chan->getName(),cmd._params[1]}), res);
                }
            }
            // Target is a user
            else
