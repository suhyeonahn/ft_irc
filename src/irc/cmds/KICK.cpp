     ...
     else
    {
        ...
        else
        {
            ...
            if (kicked->_joined.find(chan) == kicked->_joined.end())
                PushToRes(_user->_fd, getServReply(_user, ERR_NOTONCHANNEL, (string[]){cmd._params[0],cmd._params[1]}), res);
            //  ERR if the channel has invite-only mode set, and the requesting user is not a channel operator.
                ...
                PushToRes(_user->_fd, getServReply(_user, ERR_CHANOPRIVSNEEDED, (string[]){cmd._params[0]}), res);
            else
            {
                ...
                if (cmd._params.size() == 1)
                    PushToRes(kicked->_fd, getServReply(_user, MSG_KICK, (string[]){cmd._params[0]}), res);
                else
                    PushToRes(kicked->_fd, getServReply(_user, MSG_KICK, (string[]){cmd._params[0],cmd._params[1]}), res);
            }
        }
    }
